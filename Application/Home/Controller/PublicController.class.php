...
class PublicController extends Controller {
	 ...
	 protected function _initialize(){
        ...
        $this->login_sina_appid=C('LOGIN_SINA_APPID');	//新浪微博登录
    ...
    public function sina_login(){
    	$o = new SaeTOAuthV2(C('LOGIN_SINA_APPID') , C('LOGIN_SINA_APPKEY'));
    	$code_url = $o->getAuthorizeURL(C('LOGIN_SINA_CALLBACK'));
    ...
    public function sina_callback(){
    	...
    	if (isset($_REQUEST['code'])) {
    		...
    		$keys['redirect_uri'] = C('LOGIN_SINA_CALLBACK');
    		try {
    			$token = $o->getAccessToken( 'code', $keys );
    			//判断用户已存在
    			$map = array('status'=>1,'sinaid'=>$token['uid']);
    			$user=M('User')->where($map)->field('id,nickname')->find();
    			if(!$user){
    				$c = new SaeTClientV2( C('LOGIN_SINA_APPID') , C('LOGIN_SINA_APPKEY') , $token);
    				$uid_get = $c->get_uid();
    				$user_message = $c->show_user_by_id( $uid_get['uid']);//根据ID获取用户等基本信息
    				$data['nickname']=$user_message['name'];
    				$data['password']=think_ucenter_md5($user_message['id']);
    				$data['sinaid'] = $user_message['id'];
    ...
    protected function cellcode($cell){
        ...
        // 设置请求参数
        $req = $request->setSmsTemplateCode('SMS_10990922')
            ->setRecNum($cell)
    ...
    public function bindcode($cell){
        ...
        // 设置请求参数
        $req = $request->setSmsTemplateCode('SMS_10990926')
            ->setRecNum($cell)
            ->setSmsParam(json_encode($smsParams))
