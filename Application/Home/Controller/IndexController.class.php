    	...
    	if(IS_AJAX){
            ...
            if($period[0] || $period[1]){
                foreach ($period as $k=>$v){
                    ...
                    $period_list[$k]['user']=is_numeric(get_user_name($v["uid"])) ? substr_replace(get_user_name($v["uid"]),'****',3,4) : get_user_name($v["uid"]);//get_user_name($v["uid"]);
    		...
    		$this->assign('slider',M('slider')->where('status=1 and type=101')->order('id desc')->select());
