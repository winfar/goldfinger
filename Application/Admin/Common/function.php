 ...
 * @return string
 */
function get_slider_StatusValue($startTime,$endTime){
    $currTime = time();
    if($currTime < $startTime ){
        return '未开始';
    }elseif ( $currTime > $startTime && $currTime < $endTime) {
        return '进行中';
    }else{
        return '已结束';
    }
 ...
 * 
 * 获取发布平台的状态
 * 1.无参数传入返回发布平台状态的array
 * @param int $type 
 * @return array|bool|string
 */
function get_slider_publish($type=-1)
{
    if($type > 0){
        return parse_slider_publish($type);
    }else{
       $_pubulish =  array();
        foreach (range(1, 3) as $key => $number) {
            $_pubulish[]=array('id'=> $number, 'publish'=> parse_slider_publish($number))  ;
        }
        return $_pubulish;
    }
    return false;
...
function check_slider_publish($publish,$num){

    $binary  = decbin($publish);
    $result = 0;
    switch ($num){
        case 1:
            $result = substr($binary,-1);
            break;
        case 2:
            $result = substr($binary,-2,-1);
            break;
    }
 ...
 * @return bool|string
 */
function get_slider_Allpublish($type=-1)
{
    if($type > 0){
        return parse_slider_publish($type);
    }
...
function get_order_status_list()
{
    $array = array(
        array('id' => 0,'name' => '未领取' ),
        array('id' => 100,'name' => '未发货' ),
        array('id' => 101,'name' => '已发货' ),
        array('id' => 102,'name' => '已收货' ),
        );
    return $array;
}

/**
 * 物流状态  已晒单状态 显示为【已收货】状态
 * @param $type
 */
function get_logistics_status($type)
{
  return  get_order_status($type) === '已晒单' ? '已收货' : get_order_status($type) ;
}
...
function get_pkType($pid)
{
    ...
    $map['p.id']=$pid;
    $house = M('shop_period')->field('h.*')->table('__SHOP_PERIOD__ p,__HOUSE_MANAGE__ h')->where('p.house_id=h.id')->where($map)->find();

    if($house){
        return $house['ispublic']==1?"私密":"公开";
    }
    else{
        return '-';
    }
...
function getPkPersonCount($pid)
{
    ...
    $map['p.id']=$pid;
    $peoplenum = M('shop_period')->field('c.peoplenum')->table('__SHOP_PERIOD__ p,__HOUSE_MANAGE__ h,__PKCONFIG__ c')->where('p.house_id=h.id and h.pksetid=c.id')->where($map)->find();

    if($peoplenum){
        return $peoplenum['peoplenum'];
    }
    else{
        return 0;
    }
...
function purchaseorderstatus($type)
{
    switch ( $type ) {
        case 1  :
            return '已采购';
 ...
 * 提供获取有效状态的品牌列表
 */
function get_brand($status = 1)
{
    return M('brand')->where('status >= ' . $status)->field('id,title')->select();
}
...
function getRedEnvelopeCountById($redenvelopeId){
    $map['red_envelope_id'] = $redenvelopeId;
    $count = M('red_envelope_record')->where($map)->count();
    return $count;
}
...
function getNotificationType($type_id)
{
    switch ( $type_id ) {
        case 0  :
            return '公共消息';
